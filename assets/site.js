(() => {
  ...
  if (!reduced && 'IntersectionObserver' in window) {
    ...
    revealItems.forEach((el, i) => {
      if (el.classList.contains('step') || el.classList.contains('cap') || el.classList.contains('proofItem')) {
        el.style.transitionDelay = `${Math.min(i % 7, 4) * 55}ms`;
      }
      io.observe(el);
    });
  ...
  const render = now => {
    ...
    if (ss && system && laptop && phone) {
      const p = localProgress(ss);
      const e = ease(p);
      const center = Math.sin(p * Math.PI);
      const travel = (p - .5) * 2;
      const deviceX = travel * (coarse ? 90 : 150);
      const phoneX = travel * (coarse ? -135 : -210);
      laptop.style.transform = `translate3d(${deviceX}px,${-center * (coarse ? 40 : 75)}px,0) rotateY(${travel * -14}deg) rotateZ(${travel * 1.8}deg) scale(${1 + center * .035})`;
      phone.style.transform = `translate3d(${phoneX}px,${center * (coarse ? -70 : -115)}px,0) rotateY(${travel * 24}deg) rotateZ(${travel * -5}deg) scale(${1 + center * (coarse ? .10 : .14)})`;
      orbitOne.style.transform = `translate(-50%,-50%) rotate(${p * 280}deg) scale(${.82 + center * .24})`;
      orbitTwo.style.transform = `translate(-50%,-50%) rotate(${p * -220}deg) scale(${.96 + center * .18})`;
    ...
    document.querySelectorAll('.industryLine span').forEach((el,i) => {
    ...
    if(ss && system && laptop && phone && coarse){
      const p=localProgress(ss), wave=Math.sin(p*Math.PI), dir=scrollDirection;
      laptop.style.transform=`translate3d(${(p-.5)*210}px,${-wave*95}px,0) rotateY(${(p-.5)*-20}deg) rotateZ(${(p-.5)*2.8}deg) scale(${1+wave*.055})`;
      phone.style.transform=`translate3d(${(p-.5)*-270}px,${-wave*150}px,0) rotateY(${(p-.5)*32}deg) rotateZ(${(p-.5)*-7}deg) scale(${1+wave*.15})`;
      root.style.setProperty('--system-aura',(1+wave*.22+speed*.08).toFixed(3));
      root.style.setProperty('--phone-glint',`${-100+p*300}%`);
      root.style.setProperty('--screen-glint',`${-120+p*280}%`);
      root.style.setProperty('--system-bg-x',`${(p-.5)*-90+smoothVelocity*7}px`);
      root.style.setProperty('--system-bg-y',`${-wave*42+dir*speed*8}px`);
    }
